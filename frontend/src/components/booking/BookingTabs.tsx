...
type Tab = 'TRANSFER' | 'DISPOSITION';
  ...
  function LocationIcon({ type }: { type: string }) {
    if (type === 'Aéroport') return (
      <svg className="w-5 h-5 text-brand-600 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5}>
        <path d="M22 16.5H2l4-8 4 2 2-6 2 6 4-2 4 8z" />
  ...
  function useLocationSearch(value: string, routes: any[]) {
  ...
  function LocationInput({
    ...
    const results = useLocationSearch(value, routes);
      ...
      <div className="relative z-[100]" ref={ref}>
        ...
        {open && results.length > 0 && (
          <div className="absolute z-[9999] top-full mt-2 w-full bg-white rounded-2xl shadow-2xl border border-ink-100 overflow-hidden">
            ...
            <ul
              className="overflow-y-auto divide-y divide-ink-50"
    ...
    <div className="w-full max-w-4xl bg-white rounded-3xl p-6 lg:p-8 shadow-2xl text-left">
      ...
      <div className="bg-ink-100 rounded-full p-1.5 flex gap-1 mb-6">
        <button
          ...
          className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-full font-semibold text-sm transition-all duration-400 ${
            tab === 'TRANSFER'
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <input
              ...
              className="w-full px-4 py-3.5 border border-ink-200 rounded-full text-sm font-medium focus:outline-none focus:border-brand-500"
              style={{ color: '#111827' }}
            />
            ...
            <button
              onClick={search}
              className="bg-brand-600 hover:bg-brand-700 text-white font-semibold py-3.5 rounded-full transition hover:scale-[1.02] shadow-lg shadow-brand-600/30"
            >
              {t('search')}
            </button>
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <select
              ...
              className="w-full px-4 py-3.5 border border-ink-200 rounded-full text-sm font-medium focus:outline-none focus:border-brand-500 bg-white"
              style={{ color: '#111827' }}
            >
