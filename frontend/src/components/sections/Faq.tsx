        ...
        <div className="space-y-3">
          {items.map((item, i) => (
            <div key={i} style={{ transitionDelay: `${i * 80}ms` }}
              className={`reveal border rounded-2xl transition-all duration-300 overflow-hidden ${open === i ? 'border-brand-200 bg-brand-50/40' : 'border-ink-100 bg-white hover:border-ink-200'}`}>
